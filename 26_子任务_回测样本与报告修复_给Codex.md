...
- 对照真跑：`--limit 300` 最终有效样本从 36 提升到 **300**，6 个回测期均为 300 只。
- 失败分桶：元数据候选 600；成立太短预筛 32；净值请求 300；抓取成功 300；抓取失败 0；抓取后历史太短 0；最终有效 300。
- 结论：首轮 36 只的主因不是抓取失败，也不是普遍成立太短，而是旧 `_load_navs()` 写死要求净值始于 `2014-01-01`，比最早 2019 as-of 所需的 2016 年底门槛严格约 3 年。
- 报告已区分“各维 IC≤0”与“无 IC 数据”，第 3 节窗口胜负小结已补全；未修改任何配置权重。
