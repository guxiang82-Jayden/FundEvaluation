...
- 实测确认 `fund_portfolio_hold_em` 的 Q2/Q4 返回全持仓，Q1/Q3 为前十大；
  脚本严格只解析 Q2/Q4，未用季报前十大冒充全持仓。
- 新增 `scripts/build_returngap_cdim.py`：
  - 按基金/年份缓存全持仓；
  - 汇总跨基金去重股票池，每只股票只抓一次并长期缓存；
  - 股票行情使用新浪前复权源，失败时降级东财；
  - 港股、北交所异常、停牌/退市等缺失计入持仓覆盖率；
  - 单期覆盖低于 70% 作废，最多使用最近 4 个半年区间；
  - 支持 `--limit/--start-year/--end-year/--refresh/--checkpoint`。
- `cdim.py` 已接入 C3。原始审计值保留，但只有
  `return_gap_n_periods >= 2` 且 `holdings_coverage >= 70%` 才进入评分。

## 30只真实验收

- 全持仓可得：30/30。
- 去重股票：3046 只；行情可用 3026 只。
- C3 有效覆盖：30/30（100%）。
- 有效半年期：120/120；因覆盖不足作废 0 期。
- ReturnGap 分布：min -14.67% / 中位 -1.33% / max +13.98%；
  30 只中 80% 落在 ±5% 半年均值区间内。
- 代表样本（均为 4 个有效半年期）：
  - `000063` 长盛电子信息主题A：实际 6.94%，静态持仓 21.61%，gap -14.67%。
  - `000001` 华夏成长：实际 9.24%，静态持仓 14.85%，gap -5.61%。
  - `000006` 西部利得量化成长A：实际 7.31%，静态持仓 8.65%，gap -1.34%。
  - `000020` 景顺长城品质投资A：实际 9.63%，静态持仓 6.09%，gap +3.53%。
  - `000066` 诺安鸿鑫A：实际 13.77%，静态持仓 -0.21%，gap +13.98%。
- 极端样本持仓覆盖仍为 98%~100%，不是缺失行情导致；保留原始值参与同类分位，
  后续可在扩大样本后评估 winsorize，但本任务不改评分逻辑。
- `run_monthly.py --limit 30` 跑通：ReturnGap 命中 30 只，评分权重覆盖率均值 98%。
