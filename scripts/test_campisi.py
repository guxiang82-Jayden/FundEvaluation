...
def test_campisi():
    ...
    print(f"  五因子构造 ✓ {F.shape}")

    # 纯债: 因子线性组合 + 小alpha + 小噪声 → 高R², alpha可信
    ...
    print(f"  纯债: R²={rp['r2']:.3f} high, alpha年化={rp['alpha_ann']:.2%} ✓")

    # 含权: 大独立噪声(权益部分债券因子无法解释) → 低R², 标低置信度
    ...
    print(f"  含权: R²={rm['r2']:.3f} {rm['confidence']} ✓")
    ...
    assert 70 < len(wk) < 85
    print(f"  日→周频: {len(nav)}日→{len(wk)}周 ✓")
